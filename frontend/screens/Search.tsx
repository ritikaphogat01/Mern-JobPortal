...
export const Search: React.FC<SearchProps> = ({ 
  ...
  const suggestions = useMemo(() => {
    ...
    const matches: { label: string; subLabel: string; type: 'category' | 'role' }[] = [];
    
    // Match based on category hierarchy
    categoriesData.forEach(cat => {
      // 1. Parent Category match
      if (cat.name.toLowerCase().includes(q)) {
        const subCatNames = cat.subCategories?.map((s: any) => s.name.toLowerCase()) || [];
  ...
  const filteredJobs = React.useMemo(() => {
    const filtered = jobsData.filter(job => {
      ...
      const parentCat = categoriesData.find(c => 
         ...
         c.subCategories?.some((s: any) => s.name.toLowerCase() === (job.category || '').toLowerCase())
      );
      ...
      if (fullCategory && fullCategory.subCategories) {
        fullCategory.subCategories.forEach((sub: any) => {
      ...
      const isMatch = (val: string, target: string) => normalize(val).includes(normalize(target));
