...
interface MenuProps {
  onBack: () => void;
  onDownload: () => void;
  onNavigate: (screen: AppScreen, extra?: string) => void;
  hasApprovedJob?: boolean;
  hasPaidJob?: boolean;
}

export const Menu: React.FC<MenuProps> = ({ onBack, onDownload, onNavigate, hasApprovedJob = false, hasPaidJob = false }) => {
  const [showLogoutConfirm, setShowLogoutConfirm] = useState(false);
  const [candidateName, setCandidateName] = useState<string | null>(null);
  const [candidateMobile, setCandidateMobile] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    if (mobile) {
      fetch('/api/candidates')
        .then(res => res.json())
        .then(data => {
          const cand = data.find((c: any) => c.mobile === mobile);
      ...
      <div className="p-6">
        ...
        <div className="space-y-4">
          ...
          {modules.map((m, idx) => (
            <div
              key={idx}
              onClick={() => m.screen && onNavigate(m.screen as AppScreen, (m as any).type)}
