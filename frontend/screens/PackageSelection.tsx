...
import { PricingPlan, Coupon, AppScreen } from '../types';
...
interface PackageSelectionProps {
  ...
  isBoosted?: boolean;
  onBack: () => void;
  onNext: (context: { planId: string | null; planName: string | undefined; amount: number | undefined; isBoosted?: boolean; couponCode?: string; discount?: number; planCredits?: number }) => void;
  onNavigate?: (s: AppScreen) => void;
}

export const PackageSelection: React.FC<PackageSelectionProps> = ({ jobId, isBoosted, onBack, onNext }) => {
  ...
  useEffect(() => {
    ...
    fetch('/api/plans')
      .then(res => res.json())
      .then(data => {
        const recruiterPlans = data.filter((p: any) => p.role === 'EMPLOYER' || p.role === 'Recruiter');
  ...
  const getPlanCredits = (plan: PricingPlan): number => {
    const p = plan as any;
    // Admin form saves as credits.jobPosts
    if (p.credits) {
      if (typeof p.credits.jobPosts === 'number' && p.credits.jobPosts > 0) return p.credits.jobPosts;
      if (typeof p.credits.jobPosts === 'string') {
        const n = parseInt(p.credits.jobPosts);
        if (!isNaN(n) && n > 0) return n;
      }
      // Also check .jobs (legacy)
      if (typeof p.credits.jobs === 'number' && p.credits.jobs > 0) return p.credits.jobs;
      if (typeof p.credits === 'number' && p.credits > 0) return p.credits;
    }
  ...
  const handleCouponClick = () => {
    ...
    const plan = plans.find(p => (p as any)._id === selectedPlan || p.id === selectedPlan);
          ...
          {loading ? (
            ...
            <>
              <div className="flex gap-5 overflow-x-auto pb-8 snap-x no-scrollbar -mx-6 px-6">
                {plans.map((plan) => (
                  <div
                    key={(plan as any)._id || plan.id}
                    onClick={() => {
                      const id = (plan as any)._id || plan.id;
                      setSelectedPlan(id);
                      // Reset coupon if new plan is selected and it's too cheap
                      if (getPlanPrice(plan) <= 200) setSelectedCoupon(null);
                    }}
                    className={`flex-shrink-0 w-[300px] snap-center p-8 rounded-[3.5rem] border-2 transition-all duration-500 relative flex flex-col cursor-pointer ${selectedPlan === ((plan as any)._id || plan.id)
                  ...
                  >
                    ...
                    <div className={`mt-auto w-full h-12 rounded-2xl flex items-center justify-center gap-2 transition-all ${selectedPlan === ((plan as any)._id || plan.id)
                        ? 'bg-primary text-white shadow-lg shadow-primary/30'
                        : 'bg-[#FAFBFF] text-gray-400'
                      }`}>
                      <span className="text-[10px] font-black uppercase tracking-widest">
                        {selectedPlan === ((plan as any)._id || plan.id) ? 'Selected Plan' : 'Select'}
                      </span>
                    </div>
                  </div>
                ))}
      ...
      <div className="bg-white/90 backdrop-blur-xl border-t border-gray-100 p-8 flex flex-col items-center gap-6 safe-bottom fixed bottom-0 left-0 right-0 z-30 max-w-md mx-auto">
           ...
           <div className="flex items-center gap-2 px-4 py-1.5 bg-emerald-500/10 rounded-full border border-emerald-500/20">
              ...
              <p className="text-[9px] font-black text-emerald-600 uppercase tracking-widest">You saved extra ₹{
                selectedCoupon.discountType === 'PERCENT' 
                  ? Math.round(getPlanPrice(plans.find(p => (p as any)._id === selectedPlan || p.id === selectedPlan)!) * selectedCoupon.value / 100) 
                  : selectedCoupon.value
              }!</p>
           </div>
        )}
        ...
        >
          <span className="material-icons-round">lock</span> {
            selectedCoupon 
              ? `Pay ₹${getPlanPrice(plans.find(p => (p as any)._id === selectedPlan || p.id === selectedPlan)!) - (selectedCoupon.discountType === 'PERCENT' ? Math.round(getPlanPrice(plans.find(p => (p as any)._id === selectedPlan || p.id === selectedPlan)!) * selectedCoupon.value / 100) : selectedCoupon.value)}`
              : 'Buy Now & Post'
          }
        </button>
        <div className="flex items-center gap-4">
          ...
          <div className="bg-white w-full max-w-md rounded-t-[3rem] p-8 pb-12 animate-slide-up shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4 max-h-[60vh] overflow-y-auto no-scrollbar py-2">
              {coupons.length === 0 ? (
                ...
                coupons.map(coupon => (
                  <button
                    key={(coupon as any)._id || coupon.id}
