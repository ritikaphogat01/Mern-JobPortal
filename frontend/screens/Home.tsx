...
import { Job, Category, AppScreen, Notification } from '../types';

interface HomeProps {
  onSelectCategory: (cat: Category) => void;
  onSelectJob: (job: Job) => void;
  onSearch: (jobType?: string, qualification?: string) => void;
  onDownload: () => void;
...
export const Home: React.FC<HomeProps> = ({ 
  ...
  const [toastNotif, setToastNotif] = useState<{show: boolean, title: string, message: string}>({show: false, title: '', message: ''});
    ...
    <div className="animate-fade-in pb-32">
      ...
      <section className="px-5 mb-10">
        ...
        <div className="grid grid-cols-2 gap-4">
           {[
             { name: 'Full-Time', icon: 'schedule', filter: (j: Job) => j.type === 'Full-Time' },
             { name: 'Part-Time', icon: 'history', filter: (j: Job) => j.type === 'Part-Time' },
             { name: 'Internship', icon: 'school', filter: (j: Job) => j.type === 'Internship' },
             { name: 'Contract', icon: 'assignment', filter: (j: Job) => j.type === 'Contract' },
             { name: 'Freelance', icon: 'draw', filter: (j: Job) => j.type === 'Freelance' },
              { name: 'Remote', icon: 'home_work', filter: (j: Job) => j.type === 'Remote' || (j.workMode || '').toLowerCase().includes('remote') || j.location.toLowerCase().includes('remote') }
           ].map(t => {
