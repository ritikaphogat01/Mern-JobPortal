...
export const RecruiterDashboard: React.FC<RecruiterDashboardProps> = ({ onBack, onPostJob, onNavigate }) => {
  const [activeTab, setActiveTab] = useState<'overview' | 'applicants' | 'profile' | 'vacancies' | 'billing'>('overview');
  ...
  const [showStatusNotif, setShowStatusNotif] = useState<{show: boolean, status: string, name: string}>({show: false, status: '', name: ''});
  ...
  React.useEffect(() => {
    ...
    fetch('/api/admin/jobs')
      ...
      .then(data => {
        if (Array.isArray(data)) {
          const filtered = data.filter((j: any) => String(j.recruiterEmail || '').trim().toLowerCase() === recruiterEmail);
          setMyJobs(filtered);
        }
      })
      .catch(console.error)
    ...
    fetch('/api/transactions')
      ...
      .then(data => {
        if (Array.isArray(data)) {
           const filtered = data.filter((t: any) => String(t.userEmail || '').trim().toLowerCase() === recruiterEmail);
           setTransactions(filtered);
        }
      });
  }, []);

  const updateStatus = async (id: string, newStatus: string, applicant: any) => {
    try {
      ...
      <div className="bg-white p-5 rounded-[2rem] border border-gray-100 shadow-card">
        ...
        <div className="grid grid-cols-4 gap-2">
          {[
            ...
            <button key={a.tab} onClick={() => setActiveTab(a.tab as any)} className="flex flex-col items-center gap-2 active:scale-90 transition-all group">
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        onChange={e => {
                           ...
                           setProfileForm((f: any) => ({ ...f, [field.key]: val }));
                        }} 
  ...
  return (
    <div className="animate-fade-in bg-[#FAFBFF] min-h-screen pb-32 flex flex-col">
      ...
      >
        {[{ id: 'overview', label: 'Overview', icon: 'dashboard' },{ id: 'vacancies', label: 'My Vacancies', icon: 'work' },{ id: 'applicants', label: 'Applicants', icon: 'groups' },{ id: 'billing', label: 'Billing', icon: 'receipt_long' },{ id: 'profile', label: 'My Details', icon: 'business' }].map(tab => (<button key={tab.id} onClick={() => setActiveTab(tab.id as any)} className={`flex items-center gap-2 px-6 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all shrink-0 ${activeTab === tab.id ? 'bg-accent text-white shadow-lg' : 'text-gray-400 bg-gray-50'}`}><span className="material-icons-round text-base">{tab.icon}</span>{tab.label}</button>))}
      </nav>
      <main className="p-6">
         ...
         {selectedJob && (
           <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-accent/80 backdrop-blur-md animate-fade-in overflow-y-auto"><div className="bg-white w-full max-w-md rounded-[3rem] shadow-2xl animate-slide-up relative my-auto"><div className="p-8 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white z-10 rounded-t-[3rem]"><div><h3 className="text-xl font-display font-black text-accent leading-tight">Review Vacancy</h3><p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mt-1">Posted Content Details</p></div><button onClick={() => setSelectedJob(null)} className="w-10 h-10 rounded-2xl bg-gray-50 text-gray-500 flex items-center justify-center"><span className="material-icons-round">close</span></button></div><div className="p-8 space-y-6 max-h-[60vh] overflow-y-auto no-scrollbar"><div className="flex items-center gap-5 pb-6 border-b border-gray-50"><div className="w-16 h-16 rounded-2xl bg-primary/5 flex items-center justify-center border border-primary/10 shrink-0">{selectedJob.companyLogo ? (<img src={selectedJob.companyLogo} className="w-full h-full object-cover rounded-2xl" alt="Logo" />) : (<span className="material-icons-round text-primary text-3xl">apartment</span>)}</div><div><h4 className="text-xl font-display font-black text-accent">{selectedJob.title}</h4><p className="text-[10px] font-black text-primary uppercase tracking-widest mt-0.5">{selectedJob.company}</p></div></div><div className="grid grid-cols-2 gap-4"><div className="p-4 bg-gray-50 rounded-2xl"><p className="text-[8px] font-black text-gray-400 uppercase mb-1">Salary Range</p><p className="text-[11px] font-bold text-accent">{selectedJob.salary || 'Competitive'}</p></div><div className="p-4 bg-gray-50 rounded-2xl text-right"><p className="text-[8px] font-black text-gray-400 uppercase mb-1">Work Mode</p><p className="text-[11px] font-bold text-accent">{selectedJob.location} ({selectedJob.workMode || 'Onsite'})</p></div></div><div><p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-3">Job Description</p><p className="text-xs text-gray-600 font-medium leading-relaxed bg-gray-50/50 p-6 rounded-2xl border border-gray-50 whitespace-pre-wrap italic">"{selectedJob.description}"</p></div><div className="space-y-4"><p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Requirements</p><div className="grid grid-cols-1 gap-2">{selectedJob.requirements?.map((req: string, i: number) => (<div key={i} className="flex items-center gap-2 px-4 py-3 bg-white border border-gray-100 rounded-xl"><span className="material-icons-round text-primary text-sm">check_circle</span><span className="text-[10px] font-bold text-gray-700">{req}</span></div>))}</div></div></div><div className="p-8 pt-0"><button onClick={() => setSelectedJob(null)} className="w-full py-5 bg-accent text-white rounded-[2rem] text-[10px] font-black uppercase tracking-widest shadow-xl active:scale-95 transition-all">Dismiss View</button></div></div></div>
         )}
      </main>
