...
export const JobDetails: React.FC<JobDetailsProps> = ({ job, onBack, favorites, onToggleFavorite, onShare }) => {
    ...
    fetch('/api/applications')
      ...
      .then(data => {
        const count = data.filter((a: any) => 
          a.jobId === job.id || a.jobId === (job as any)._id
        ).length;
      ...
      <main className="flex-1 overflow-y-auto no-scrollbar pb-10">
        {(job as any).imageUrl && (
          <div className="w-full h-48 bg-gray-100 overflow-hidden relative z-0">
            <img src={(job as any).imageUrl} alt="Job Banner" className="w-full h-full object-cover" />
          </div>
        )}
        <div className="px-6 pt-4 pb-6 relative z-10 w-full mt-[-20px] bg-white rounded-t-[1.5rem]">
          ...
          <div className="space-y-4 mb-8">
            ...
            <div className="flex items-center gap-4">
              ...
              <span className="text-[15px] text-gray-800">{job.type} {(job as any).workMode && `(${(job as any).workMode})`}</span>
            </div>
            <div className="flex items-center gap-4">
            ...
            <div className="pt-8 space-y-5">
              ...
              <div className="flex justify-between items-start">
                ...
                <span className="text-accent text-[15px] font-semibold flex-1 text-left">
                  {(job as any).workMode || (job.type.toLowerCase().includes('remote') ? 'Remote' : 'On-site')}
                </span>
              </div>
            </div>
          </div>
