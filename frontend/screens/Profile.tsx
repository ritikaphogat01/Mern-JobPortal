...
export const Profile: React.FC<ProfileProps> = ({ onBack, onNavigate, hasPaidJob = false }) => {
  ...
  const handlePhotoDelete = async () => {
    ...
    try {
      ...
      const existing = candidates.find((c: any) => c.mobile === profile.mobile);
      if (existing) {
  ...
  useEffect(() => {
    ...
    fetch('/api/candidates')
      .then(res => res.json())
      .then(data => {
        const myProfile = data.find((c: any) => c.mobile === mobile);
        if (myProfile) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (candidateId) {
        const resList = await fetch('/api/candidates');
        const candidates = await resList.json();
        existing = candidates.find((c: any) => (c._id || c.id) === candidateId);
      } else {
        // Fallback search by mobile if no ID yet
        const resList = await fetch('/api/candidates');
        const candidates = await resList.json();
        existing = candidates.find((c: any) => c.mobile === profile.mobile);
      }
  ...
  const quickLinks: { label: string; icon: string; screen: AppScreen }[] = [
