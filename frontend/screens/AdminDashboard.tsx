...
type MenuSection =
  ...
  | 'Coupons' | 'Companies' | 'Candidates' | 'Jobs'
  | 'Employer' | 'Audit Logs' | 'Settings' | 'Categories';
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ onBack }) => {
  ...
  const [jobUpdating, setJobUpdating] = useState<string | null>(null);
  ...
  const handlePlanSave = async () => {
    ...
    try {
      if (editingPlan) {
        const res = await fetch(`/api/plans/${(editingPlan as any)._id || editingPlan.id}`, {
          method: 'PUT',
        ...
        if (res.ok) {
          ...
          setPlans(prev => prev.map(p => ((p as any)._id || p.id) === ((editingPlan as any)._id || editingPlan.id) ? updated : p));
        }
      } else {
  ...
  const handlePlanDelete = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) {
        setPlans(prev => prev.filter(p => ((p as any)._id || p.id) !== id));
  ...
  const openEditCategoryModal = (cat: any) => { setEditingCategory(cat); setCategoryForm({ name: cat.name, icon: cat.icon || 'category', imageUrl: cat.imageUrl || '' }); setShowCategoryModal(true); };
  ...
  const handleSubCategoryDelete = async (subName: string) => {
    if (!activeCategoryId) return;
    try {
      const cat = categories.find(c => (c._id || c.id) === activeCategoryId);
      if (!cat) return;
      
      const updatedSubCats = (cat.subCategories || []).filter((s: any) => s.name !== subName);
      const res = await fetch(`/api/categories/${activeCategoryId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ subCategories: updatedSubCats })
  ...
  const openEditSubModal = (sub: UserSubscription) => {
    setEditingSub(sub);
    setSubForm({
      userId: sub.userId,
  ...
  const handleSubSave = async () => {
    ...
    try {
      if (editingSub) {
        const id = (editingSub as any)._id || editingSub.id;
        ...
        if (res.ok) {
          const updated = await res.json();
          setSubscriptions(prev => prev.map(s => ((s as any)._id || s.id) === id ? updated : s));
        }
      } else {
  ...
  const handleSubDelete = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) {
        setSubscriptions(prev => prev.filter(s => ((s as any)._id || s.id) !== id));
  ...
  const handleTxSave = async () => {
    ...
    try {
      if (editingTx) {
        const id = (editingTx as any)._id || editingTx.id;
        ...
        if (res.ok) { const updated = await res.json(); setTransactions(prev => prev.map(t => ((t as any)._id || t.id) === id ? updated : t)); }
      } else {
  ...
  const handleTxDelete = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) { setTransactions(prev => prev.filter(t => ((t as any)._id || t.id) !== id)); setDeleteTxConfirmId(null); }
  ...
  const handleCouponSave = async () => {
    ...
    try {
      if (editingCoupon) {
        const id = (editingCoupon as any)._id || editingCoupon.id;
        const res = await fetch(`/api/coupons/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
        if (res.ok) { const updated = await res.json(); setCoupons(prev => prev.map(c => ((c as any)._id || c.id) === id ? updated : c)); }
      } else {
  ...
  const handleCouponDelete = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) { setCoupons(prev => prev.filter(c => ((c as any)._id || c.id) !== id)); setDeleteCouponConfirmId(null); }
  ...
  const openEditCompanyModal = (c: any) => {
    setEditingCompany(c);
  ...
  const handleCompanySave = async () => {
    ...
    try {
      ...
      if (editingCompany) {
        const id = (editingCompany as any)._id || editingCompany.id;
        const res = await fetch(`/api/companies/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
        if (res.ok) { const updated = await res.json(); setCompanies(prev => prev.map(c => ((c as any)._id || c.id) === id ? updated : c)); }
      } else {
  ...
  const handleCompanyDelete = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) { setCompanies(prev => prev.filter(c => ((c as any)._id || c.id) !== id)); setDeleteCompanyConfirmId(null); }
  ...
  const handleCandidateSave = async () => {
    ...
    try {
      if (editingCandidate) {
        const id = (editingCandidate as any)._id || editingCandidate.id;
        ...
        if (res.ok) { const updated = await res.json(); setCandidates(prev => prev.map(c => ((c as any)._id || c.id) === id ? updated : c)); }
      } else {
  ...
  const handleCandidateDelete = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) { setCandidates(prev => prev.filter(c => ((c as any)._id || c.id) !== id)); setDeleteCandidateConfirmId(null); }
  ...
  const [deleteFlagConfirmId, setDeleteFlagConfirmId] = useState<string | null>(null);
  ...
  const handleFlagSave = async () => {
    ...
    try {
      if (editingFlag) {
        const id = (editingFlag as any)._id || editingFlag.id;
        const res = await fetch(`/api/flaggeditems/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(flagForm) });
        if (res.ok) { const updated = await res.json(); setFlaggedItems(prev => prev.map(f => ((f as any)._id || f.id) === id ? updated : f)); }
      } else {
  ...
  const handleFlagDelete = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) { setFlaggedItems(prev => prev.filter(f => ((f as any)._id || f.id) !== id)); setDeleteFlagConfirmId(null); }
  ...
  const [deleteAuditConfirmId, setDeleteAuditConfirmId] = useState<string | null>(null);
  ...
  const handleAuditDelete = async (id: string) => {
    setAuditDeleting(true);
    try {
      const res = await fetch(`/api/auditlogs/${id}`, { method: 'DELETE' });
      if (res.ok) { setAuditLogs(prev => prev.filter(l => ((l as any)._id || l.id) !== id)); setDeleteAuditConfirmId(null); }
      ...
      <div className="space-y-8 animate-fade-in">
        ...
        <div className="space-y-4">
          {flaggedItems.map(flag => {
            const fId = (flag as any)._id || flag.id;
            return (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-8 rounded-[2rem] border border-gray-100 shadow-sm">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="text-sm font-bold text-gray-600 mb-2 block">Job Post Fee (â‚¹)</label>
  ...
  const handleJobVerification = async (id: string, newStatus: string) => {
    setJobUpdating(id);
    try {
      ...
      if (res.ok) {
        const updated = await res.json();
        setAdminJobs(prev => prev.map(j => ((j as any)._id || j.id) === id ? updated : j));
      }
    } catch (err) {
  ...
  const handleJobDelete = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) {
        setAdminJobs(prev => prev.filter(j => ((j as any)._id || j.id) !== id));
      }
    } catch (err) {
          ...
          <div className="flex gap-4 items-center">
            ...
            {activeCat && (
               <div className="bg-gray-50 px-6 py-3 rounded-2xl border border-gray-100 flex flex-col items-center">
                 ...
                 <p className="text-xl font-display font-black text-primary">{((activeCat as any).subCategories || []).length}</p>
               </div>
            )}
            {activeCat && (
      ...
      <div className="space-y-8 animate-fade-in">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {paidJobs.map(job => {
            ...
            const pay = (job as any).paymentInfo || {};
            return (
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <input 
                ...
                placeholder="e.g. â‚¹25,000 - â‚¹30,000" 
                className="w-full h-14 pl-4 pr-4 bg-gray-50 rounded-2xl font-bold text-sm text-gray-900 border-transparent focus:border-primary transition-all"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              >
                ...
                {!categories.some((c: any) => c.name === 'Blue-Collar') && <option value="Blue-Collar">Blue-Collar</option>}
              </select>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-6">
            {transactions.slice(0, 4).map(txn => (
              <div key={txn.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl">
                ...
                <p className="text-sm font-black text-emerald-600">+â‚¹{txn.amount}</p>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex overflow-x-auto no-scrollbar">
            ...
            <div className="p-10 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <select
                    value={planForm.role}
                    onChange={e => setPlanForm(f => ({ ...f, role: e.target.value as PlanRole }))}
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2">Price (â‚¹) *</label>
                  <input
                    type="number"
                    min={0}
      ...
      {/* â”€â”€ DELETE PLAN CONFIRM MODAL â”€â”€ */}
      {deleteConfirmId && (
      ...
      {/* â”€â”€ SUBSCRIPTION CREATE / EDIT MODAL â”€â”€ */}
      {showSubModal && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 bg-accent/70 backdrop-blur-md animate-fade-in">
          <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl animate-slide-up overflow-y-auto max-h-[90vh]">
            <div className="p-10 border-b border-gray-100 flex items-center justify-between">
            ...
            <div className="p-10 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <select
                    value={subForm.role}
                    onChange={e => setSubForm(f => ({ ...f, role: e.target.value as PlanRole }))}
                    className="w-full border border-gray-200 rounded-2xl px-5 py-4 text-sm font-bold text-gray-800 focus:outline-none focus:ring-2 focus:ring-primary/30 bg-gray-50"
                  >
                    <option value="EMPLOYEE">EMPLOYEE</option>
                    <option value="EMPLOYER">EMPLOYER</option>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <datalist id="plan-names-list">
                    {plans.map(p => <option key={(p as any)._id || p.id} value={p.name} />)}
                  </datalist>
                </div>
                <div>
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2">Status</label>
                  <select
                    ...
                    onChange={e => setSubForm(f => ({ ...f, status: e.target.value as UserSubscription['status'] }))}
                    className="w-full border border-gray-200 rounded-2xl px-5 py-4 text-sm font-bold text-gray-800 focus:outline-none focus:ring-2 focus:ring-primary/30 bg-gray-50"
                  >
                    <option value="Active">Active</option>
      ...
      {/* â”€â”€ DELETE SUBSCRIPTION CONFIRM MODAL â”€â”€ */}
      {deleteSubConfirmId && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-6 bg-accent/70 backdrop-blur-md animate-fade-in">
          <div className="bg-white w-full max-w-sm rounded-[3rem] p-10 shadow-2xl animate-slide-up text-center">
            <div className="w-16 h-16 bg-red-50 rounded-3xl flex items-center justify-center text-primary mx-auto mb-6">
      ...
      {/* â”€â”€ TRANSACTION CREATE / EDIT MODAL â”€â”€ */}
      {showTxModal && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 bg-accent/70 backdrop-blur-md animate-fade-in">
          <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl animate-slide-up overflow-y-auto max-h-[90vh]">
            <div className="p-10 border-b border-gray-100 flex items-center justify-between">
            ...
            <div className="p-10 space-y-6">
              <div className="grid grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <datalist id="tx-plan-names">{plans.map(p => <option key={(p as any)._id || p.id} value={p.name} />)}</datalist>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2">Amount (â‚¹)</label>
                ...
                <div>
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2">Status</label>
                  <select value={txForm.status} onChange={e => setTxForm(f => ({ ...f, status: e.target.value as Transaction['status'] }))} className="w-full border border-gray-200 rounded-2xl px-5 py-4 text-sm font-bold text-gray-800 focus:outline-none focus:ring-2 focus:ring-primary/30 bg-gray-50">
      ...
      {/* â”€â”€ DELETE TRANSACTION CONFIRM MODAL â”€â”€ */}
      {deleteTxConfirmId && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-6 bg-accent/70 backdrop-blur-md animate-fade-in">
          <div className="bg-white w-full max-w-sm rounded-[3rem] p-10 shadow-2xl animate-slide-up text-center">
            <div className="w-16 h-16 bg-red-50 rounded-3xl flex items-center justify-center text-primary mx-auto mb-6">
      ...
      {/* â”€â”€ COUPON CREATE / EDIT MODAL â”€â”€ */}
      {showCouponModal && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 bg-accent/70 backdrop-blur-md animate-fade-in">
          <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl animate-slide-up overflow-y-auto max-h-[90vh]">
            <div className="p-10 border-b border-gray-100 flex items-center justify-between">
            ...
            <div className="p-10 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <select value={couponForm.role} onChange={e => setCouponForm(f => ({ ...f, role: e.target.value as Coupon['role'] }))} className="w-full border border-gray-200 rounded-2xl px-5 py-4 text-sm font-bold text-gray-800 focus:outline-none focus:ring-2 focus:ring-primary/30 bg-gray-50">
                ...
                <div>
                  ...
                  <select value={couponForm.discountType} onChange={e => setCouponForm(f => ({ ...f, discountType: e.target.value as Coupon['discountType'] }))} className="w-full border border-gray-200 rounded-2xl px-5 py-4 text-sm font-bold text-gray-800 focus:outline-none focus:ring-2 focus:ring-primary/30 bg-gray-50">
      ...
      {/* â”€â”€ DELETE COUPON CONFIRM MODAL â”€â”€ */}
      ...
      {/* â”€â”€ COMPANY CREATE / EDIT MODAL â”€â”€ */}
      {showCompanyModal && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 bg-accent/70 backdrop-blur-md animate-fade-in">
          <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl animate-slide-up overflow-y-auto max-h-[90vh]">
            <div className="p-10 border-b border-gray-100 flex items-center justify-between">
      ...
      {/* â”€â”€ DELETE COMPANY CONFIRM â”€â”€ */}
      ...
      {/* â”€â”€ CANDIDATE CREATE / EDIT MODAL â”€â”€ */}
      {showCandidateModal && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 bg-accent/70 backdrop-blur-md animate-fade-in">
          <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl animate-slide-up overflow-y-auto max-h-[90vh]">
            <div className="p-10 border-b border-gray-100 flex items-center justify-between">
      ...
      {/* â”€â”€ DELETE CANDIDATE CONFIRM â”€â”€ */}
      ...
      {/* â”€â”€ FLAGGED ITEM CREATE / EDIT MODAL â”€â”€ */}
      {showFlagModal && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 bg-accent/70 backdrop-blur-md animate-fade-in">
          <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl animate-slide-up overflow-y-auto max-h-[90vh]">
            <div className="p-10 border-b border-gray-100 flex items-center justify-between">
      ...
      {/* â”€â”€ DELETE FLAGGED ITEM CONFIRM â”€â”€ */}
      ...
      {/* â”€â”€ AUDIT LOG CREATE MODAL â”€â”€ */}
      {showAuditModal && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 bg-accent/70 backdrop-blur-md animate-fade-in">
          <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl animate-slide-up overflow-y-auto max-h-[90vh]">
            <div className="p-10 border-b border-gray-100 flex items-center justify-between">
      ...
      {/* â”€â”€ DELETE AUDIT LOG CONFIRM â”€â”€ */}
      ...
      {/* â”€â”€ JOB DETAILS VIEW MODAL â”€â”€ */}
      {selectedJobDetails && (
              ...
              <div className="grid grid-cols-2 gap-8">
                <div>
                  ...
                  <ul className="space-y-2">
                    {selectedJobDetails.requirements?.map((req: string, i: number) => (
