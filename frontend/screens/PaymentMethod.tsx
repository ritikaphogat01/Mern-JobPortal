...
export const PaymentMethod: React.FC<PaymentMethodProps> = ({ jobId, planId, planName, amount, isBoosted, couponCode, discount = 0, planCredits = 3, onBack, onComplete }) => {
  ...
  const handleProcessPayment = async () => {
    ...
    try {
      ...
      const handlerCallback = async (response: any) => {
        try {
          setProcessing(true);
          ...
          if (verifyData.success) {
            ...
            if (couponCode) {
               try {
                 ...
                 const c = coupons.find((x: any) => x.code === couponCode);
                 if (c) {
