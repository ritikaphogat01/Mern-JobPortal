...
const App: React.FC = () => {
  ...
  const [paymentContext, setPaymentContext] = useState<{
    ...
    discount?: number;
    planCredits?: number;
  }>({ planId: null, planName: undefined, amount: undefined, isBoosted: false, planCredits: 3 });
  const [postJobCategory, setPostJobCategory] = useState<{ category: string, subCategory: string } | null>(null);
  const [allJobs, setAllJobs] = useState<Job[]>([]);
  const [activeService, setActiveService] = useState<'GOLD' | 'BOOST' | 'AD'>('GOLD');
        ...
        .then(data => {
          if (Array.isArray(data)) {
            const formatted = data.map((j: any) => ({
              ...
              imageUrl: j.imageUrl || '',
              responsibilities: Array.isArray(j.requirements) ? j.requirements : (typeof j.requirements === 'string' && j.requirements ? j.requirements.split(',').map((s: string) => s.trim()) : []),
              requirements: Array.isArray(j.skills) ? j.skills : (typeof j.skills === 'string' && j.skills ? j.skills.split(',').map((s: string) => s.trim()) : []),
    ...
    const checkJobStatus = async () => {
      try {
        ...
        if (res.ok) {
          ...
          const myJobs = allJobs.filter((j: any) => String(j.recruiterEmail || '').trim().toLowerCase() === formattedEmail);
          const approved = myJobs.some((j: any) => j.status === 'active');
          const paid = myJobs.some((j: any) => j.status === 'paid' || j.status === 'active');
  ...
  const [navigationStack, setNavigationStack] = useState<AppScreen[]>([AppScreen.ROLE_SELECTION]);
...
You have ${newRemaining} more free job posts remaining in your plan.`);
      ...
      case AppScreen.MENU:
        return <Menu onBack={handleBack} onDownload={startDownload} onNavigate={(screen, type) => {
          if (type) setActiveService(type as any);
