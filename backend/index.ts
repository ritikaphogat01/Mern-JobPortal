...
mongoose.connect(uri)
  .then(async () => {
    ...
    if (count === 0) {
      console.log('🌱 Seeding job categories (first time)...');
      const transformed = fullCategories.map(c => ({
        name: c.name,
        icon: c.icon,
        subCategories: c.subCategories
      }));
      JobCategory.insertMany(transformed)
        .then(() => console.log('✅ Categories seeded!'))
        .catch(err => console.error('❌ Seeding failed:', err.message));
    } else {
      console.log(`✅ jobcategories already has ${count} documents - skipping seed.`);
    }
  })
  .catch(err => {
...
let memoryCategories: any[] = fullCategories.map((c, i) => ({ ...c, id: `cat-${i}`, _id: `cat-${i}` }));
...
app.get('/api/applications', async (_req, res) => {
  try {
    if (isDbConnected) {
      ...
      const enrichedApps = memoryApplications.map((app: any) => {
        const job = memoryJobs.find(j => j._id === app.jobId || j.id === app.jobId);
        return { ...app, jobDetails: job };
      });
      res.json(enrichedApps.sort((a: any, b: any) => (b.createdAt || '').localeCompare(a.createdAt || '')));
    }
  }
  catch (e) {
...
app.get('/api/admin/all', async (_req, res) => {
  ...
  } catch (e: any) { res.status(500).json({ message: 'Error loading admin data', error: e.message }); }
});
...
app.post('/api/auth/reset-candidates', async (_req: Request, res: Response) => {
  try {
    if (isDbConnected) {
...
app.post('/api/auth/login-email', async (req: Request, res: Response) => {
  try {
    ...
    if (isDbConnected) {
      ...
      user = memoryCandidates.find((c: any) => c.email === email || c.mobile === email);
      if (user) userExists = true;
    }
